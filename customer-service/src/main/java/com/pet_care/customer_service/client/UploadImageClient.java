...
@FeignClient(
        name = "uploadClient", // Name of the Feign client
        url = "http://localhost:8089/api/v1/upload-service/image/upload" // Base URL for the upload service
)
public interface UploadImageClient {
     ...
     */
    @PostMapping(consumes = MediaType.MULTIPART_FORM_DATA_VALUE) // Define POST request with multipart/form-data content type
